...
def generate_compliance_dashboard_svg(repos: list, stats: dict, activity: dict) -> str:
  ...
  <text x="{width/2}" y="140" fill="{health_color}" filter="url(#strong-glow)"
        font-family="'Courier New', monospace" font-size="28" font-weight="bold" text-anchor="middle">
    SYSTEM HEALTH: {health_status}
  </text>

  <!-- Compliance Score -->
  <text x="{width/2}" y="175" fill="{TEXT_COLOR}"
        font-family="'Courier New', monospace" font-size="16" text-anchor="middle">
    COMPLIANCE SCORE: <tspan fill="{health_color}" font-weight="bold" font-size="20">{compliance_score}%</tspan>
  </text>
  ...
  <text x="90" y="280" fill="{NEON_PRIMARY}" filter="url(#glow)"
        font-family="'Courier New', monospace" font-size="16" font-weight="bold" letter-spacing="2">
    ▸ OPERATIONAL METRICS
  </text>
  ...
  <text x="90" y="490" fill="{NEON_PRIMARY}" filter="url(#glow)"
    ...
    for i, (label, status, color) in enumerate(auth_items):
        ...
        svg_parts.append(f'''
  <text x="{x}" y="{y}" fill="{TEXT_COLOR}" font-family="'Courier New', monospace" font-size="12">
